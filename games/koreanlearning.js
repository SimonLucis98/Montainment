/* Learn Korean · 6-Level Korean Learning Game */
(function () {
  'use strict';
...
.hero{
  text-align:center;padding:20px 10px 26px;
}
.hero h1{
...
/* ========== MODE SELECT ========== */
.mode-head{
  padding:4px 0 20px;flex-shrink:0;
}
.back-btn{
...
.mode-card{
  ...
  padding:24px 20px;cursor:pointer;transition:.22s;
  text-align:center;
}
.mode-card:hover{
...
.quiz-progress .big{
  color:var(--text);font-size:16px;font-weight:900;
}
.quiz-score{
...
.quiz-score .combo{
  color:var(--pink);font-size:15px;font-weight:900;
}
...
.q-text-big{
  ...
  text-shadow:0 0 30px rgba(255,155,106,.4);
  line-height:1.1;
}
...
.q-speaker{
  ...
  cursor:pointer;
  box-shadow:0 10px 40px rgba(91,158,255,.4);
  transition:.15s;
}
.q-speaker:hover{transform:scale(1.06)}
.q-speaker:active{transform:scale(.95)}
.q-speaker.playing{
  animation:speakerPulse .8s ease-in-out infinite;
}
...
.result-icon{font-size:88px;line-height:1;margin-bottom:14px;
  ...
  <div class="screen on" id="screenHome">
    <div class="home-scroll">
      <div class="hero">
        ...
        <p id="heroDesc">Choose your level. Each level has its own vocabulary, phrases and dialogue. Questions are randomized every time you enter.</p>
      </div>
...
<script>
/* ============================================================
   KOREAN MASTER — GAME ENGINE
   ============================================================ */

/* ---------- I18N (UI only, content is always in Chinese/English) ---------- */
...
const UI = {
  ...
  en: {
    ...
    modeTextDesc: 'Read the Korean, pick the correct meaning',
    modeListen: 'Listen & Pick',
    modeListenDesc: 'Listen to the Korean pronunciation, pick the correct word',
    ...
    resultSub: 'Round finished',
    againBtn: 'Play Again',
    backBtn: 'Back to Levels',
    noAudio: '⚠️ Korean voice not available on this device',
    levelNames: {
      1:'Beginner', 2:'Basic Vocabulary', 3:'Daily Phrases',
      4:'Short Dialogue', 5:'Advanced Conversation', 6:'Fluent Communication'
    }
  }
};
...
/* ---------- LEVEL 1 : 한글 기초 (Hangul basics) ---------- */
const L1_CONSONANTS = [
  { kr:'ㄱ', roman:'g/k', name:'기역' },
  { kr:'ㄴ', roman:'n',   name:'니은' },
  { kr:'ㄷ', roman:'d/t', name:'디귿' },
  { kr:'ㄹ', roman:'r/l', name:'리을' },
  { kr:'ㅁ', roman:'m',   name:'미음' },
  { kr:'ㅂ', roman:'b/p', name:'비읍' },
  { kr:'ㅅ', roman:'s',   name:'시옷' },
  { kr:'ㅇ', roman:'ng',  name:'이응' },
  { kr:'ㅈ', roman:'j',   name:'지읒' },
  { kr:'ㅊ', roman:'ch',  name:'치읓' },
  { kr:'ㅋ', roman:'k',   name:'키읔' },
  { kr:'ㅌ', roman:'t',   name:'티읕' },
  { kr:'ㅍ', roman:'p',   name:'피읖' },
  { kr:'ㅎ', roman:'h',   name:'히읗' },
  { kr:'ㄲ', roman:'kk',  name:'쌍기역' },
  { kr:'ㄸ', roman:'tt',  name:'쌍디귿' },
  { kr:'ㅃ', roman:'pp',  name:'쌍비읍' },
  { kr:'ㅆ', roman:'ss',  name:'쌍시옷' },
  { kr:'ㅉ', roman:'jj',  name:'쌍지읒' }
];

const L1_VOWELS = [
  { kr:'ㅏ', roman:'a',   name:'아' },
  { kr:'ㅑ', roman:'ya',  name:'야' },
  { kr:'ㅓ', roman:'eo',  name:'어' },
  { kr:'ㅕ', roman:'yeo', name:'여' },
  { kr:'ㅗ', roman:'o',   name:'오' },
  { kr:'ㅛ', roman:'yo',  name:'요' },
  { kr:'ㅜ', roman:'u',   name:'우' },
  { kr:'ㅠ', roman:'yu',  name:'유' },
  { kr:'ㅡ', roman:'eu',  name:'으' },
  { kr:'ㅣ', roman:'i',   name:'이' },
  { kr:'ㅐ', roman:'ae',  name:'애' },
  { kr:'ㅔ', roman:'e',   name:'에' },
  { kr:'ㅒ', roman:'yae', name:'얘' },
  { kr:'ㅖ', roman:'ye',  name:'예' },
  { kr:'ㅘ', roman:'wa',  name:'와' },
  { kr:'ㅙ', roman:'wae', name:'왜' },
  { kr:'ㅚ', roman:'oe',  name:'외' },
  { kr:'ㅝ', roman:'wo',  name:'워' },
  { kr:'ㅞ', roman:'we',  name:'웨' },
  { kr:'ㅟ', roman:'wi',  name:'위' },
  { kr:'ㅢ', roman:'ui',  name:'의' }
];

/* Syllable generator: basic consonant + vowel combos */
...
function buildSyllableList(){
  ...
  for(const c of SYLLABLE_CONSONANTS){
    for(const v of SYLLABLE_VOWELS){
      ...
      list.push({ kr, roman, name: kr, sub: roman });
    }
  }
  return list;
}
...
/* ---------- LEVEL 2 : 기초 단어 (Basic vocabulary) ---------- */
const L2 = [
  /* Animals */
  ...
  { img:'👨', kr:'형',    roman:'hyeong',  zh:'哥哥(男称)', en:'older brother' },
  { img:'👩', kr:'누나',  roman:'nuna',    zh:'姐姐(男称)', en:'older sister' },
...
/* ---------- LEVEL 5 : 고급 회화 (Advanced conversation) ---------- */
const L5 = [
  { kr:'오늘 정말 바빴어요',              zh:'今天真的很忙',                en:'Today was really busy' },
  { kr:'주말에 뭐 할 거예요?',            zh:'周末要做什么？',              en:'What will you do on the weekend?' },
  { kr:'같이 영화 보러 갈래요?',          zh:'要一起去看电影吗？',          en:'Want to go watch a movie together?' },
  { kr:'요즘 어떻게 지내세요?',           zh:'最近过得怎么样？',            en:'How have you been lately?' },
  { kr:'그 소식 들었어요?',               zh:'你听说那个消息了吗？',        en:'Did you hear that news?' },
  { kr:'정말 놀랐어요',                   zh:'真让我吃惊',                  en:'I was really surprised' },
  { kr:'생각보다 어려웠어요',             zh:'比想象中难',                  en:'It was harder than expected' },
  { kr:'다시 한번 생각해 볼게요',         zh:'我再考虑一下',                en:'Let me think about it again' },
  { kr:'제 생각에는 좋을 것 같아요',      zh:'我觉得会不错',                en:'I think it would be good' },
  { kr:'그건 좀 어려울 것 같아요',        zh:'那个可能有点难',              en:'That might be difficult' },
  { kr:'시간이 있으면 같이 가요',         zh:'有时间的话一起去',            en:'If you have time, let us go together' },
  { kr:'혹시 도와주실 수 있나요?',        zh:'能帮我一下吗？',              en:'Could you help me?' },
  { kr:'제가 잘못한 것 같아요',           zh:'我好像做错了',                en:'I think I made a mistake' },
  { kr:'다음에 또 만나요',                zh:'下次再见',                    en:'See you next time' },
  { kr:'오늘 하루 어땠어요?',             zh:'今天过得怎么样？',            en:'How was your day?' },
  { kr:'요즘 일이 많아요',                zh:'最近工作很多',                en:'I have a lot of work lately' },
  { kr:'스트레스 받지 마세요',            zh:'别给自己压力',                en:'Do not stress' },
  { kr:'건강이 제일 중요해요',            zh:'健康最重要',                  en:'Health is most important' },
  { kr:'푹 쉬세요',                       zh:'好好休息',                    en:'Get some rest' },
  { kr:'잘 하고 있어요',                  zh:'你做得好',                    en:'You are doing well' },
  { kr:'조금만 더 힘내세요',              zh:'再坚持一下',                  en:'Hang in there a bit more' },
  { kr:'제가 연락드릴게요',               zh:'我会联系您的',                en:'I will contact you' },
  { kr:'약속을 잊지 마세요',              zh:'别忘了约定',                  en:'Do not forget the appointment' },
  { kr:'무슨 일이 있었어요?',             zh:'发生了什么事？',              en:'What happened?' },
  { kr:'제 말을 이해하셨나요?',           zh:'你听懂我说的话了吗？',        en:'Did you understand what I said?' },
  { kr:'더 자세히 설명해 주세요',         zh:'请更详细地说明',              en:'Please explain in more detail' },
  { kr:'다른 의견 있으세요?',             zh:'有其他意见吗？',              en:'Any other opinions?' },
  { kr:'동의합니다',                      zh:'我同意',                      en:'I agree' },
  { kr:'반대합니다',                      zh:'我反对',                      en:'I disagree' },
  { kr:'확실하지 않아요',                 zh:'不太确定',                    en:'I am not sure' },
  { kr:'다시 연락드릴게요',               zh:'我再联系您',                  en:'I will get back to you' },
  { kr:'미리 알려 주세요',                zh:'请提前通知我',                en:'Please let me know in advance' },
  { kr:'언제 시간이 되세요?',             zh:'您什么时候有空？',            en:'When are you free?' },
  { kr:'제가 예약을 도와드릴게요',        zh:'我帮您预订',                  en:'I will help you with the reservation' },
  { kr:'문제가 생겼어요',                 zh:'出问题了',                    en:'There is a problem' },
  { kr:'곧 해결될 거예요',                zh:'很快会解决的',                en:'It will be solved soon' },
  { kr:'조금만 기다려 주세요',            zh:'请稍等',                      en:'Please wait a moment' },
  { kr:'다시 시도해 보세요',              zh:'请再试一次',                  en:'Please try again' },
  { kr:'결국 잘 될 거예요',               zh:'最终会好的',                  en:'It will work out in the end' },
  { kr:'함께 해서 즐거웠어요',            zh:'一起很开心',                  en:'It was fun doing it together' }
];

/* ---------- LEVEL 6 : 유창한 회화 (Fluent communication) ---------- */
const L6 = [
  { kr:'한국에 온 지 얼마나 됐어요?',                   zh:'来韩国多久了？',                 en:'How long have you been in Korea?' },
  { kr:'한국 생활에 적응하는 게 쉽지 않아요',           zh:'适应韩国生活不容易',             en:'Adapting to life in Korea is not easy' },
  { kr:'한국어를 배우는 게 어렵지만 재미있어요',        zh:'学韩语虽难但很有趣',             en:'Learning Korean is hard but fun' },
  { kr:'매일 조금씩 연습하면 실력이 늘 거예요',         zh:'每天练习一点就会进步',           en:'Practice a bit every day and you will improve' },
  { kr:'언어를 배우는 가장 좋은 방법은 계속 말하는 거예요', zh:'学语言最好的方法就是不停地说', en:'The best way to learn a language is to keep speaking' },
  { kr:'실수해도 괜찮아요, 그것도 배우는 과정이에요',   zh:'犯错也没关系，那也是学习过程',    en:'It is okay to make mistakes, that is part of learning' },
  { kr:'요즘 한국 문화에 푹 빠져 있어요',              zh:'最近迷上了韩国文化',              en:'I am really into Korean culture these days' },
  { kr:'이번 주말에 뭐 특별한 계획 있어요?',           zh:'这周末有什么特别计划吗？',        en:'Any special plans for this weekend?' },
  { kr:'일이 너무 많아서 쉴 시간이 없어요',            zh:'工作太多没时间休息',              en:'Too much work, no time to rest' },
  { kr:'건강을 위해서 운동을 시작했어요',              zh:'为了健康开始运动了',              en:'I started exercising for my health' },
  { kr:'요리는 처음이지만 배우고 싶어요',              zh:'虽然第一次做菜但想学',            en:'It is my first time cooking but I want to learn' },
  { kr:'내일 회의 자료를 준비해야 해요',                zh:'明天要准备会议资料',              en:'I need to prepare meeting materials for tomorrow' },
  { kr:'날씨가 추워지니까 옷을 따뜻하게 입으세요',     zh:'天气变冷了要多穿点',              en:'It is getting cold so dress warmly' },
  { kr:'이 문제에 대해 어떻게 생각하세요?',            zh:'你对这个问题怎么看？',            en:'What do you think about this issue?' },
  { kr:'제 의견을 말씀드려도 될까요?',                 zh:'我可以发表一下意见吗？',          en:'May I share my opinion?' },
  { kr:'그 결정은 신중하게 해야 할 것 같아요',         zh:'那个决定要慎重考虑',              en:'That decision should be made carefully' },
  { kr:'다른 사람의 의견도 들어보는 게 좋을 것 같아요', zh:'最好也听听别人的意见',            en:'It would be good to hear other opinions too' },
  { kr:'이 일은 제가 책임지고 처리하겠습니다',         zh:'这件事我会负责处理',              en:'I will take responsibility for this' },
  { kr:'함께 협력하면 더 좋은 결과가 나올 거예요',     zh:'一起合作会有更好的结果',          en:'Working together will yield better results' },
  { kr:'최선을 다했으니 후회는 없어요',                zh:'尽力了所以没有遗憾',              en:'I did my best so no regrets' },
  { kr:'앞으로도 잘 부탁드립니다',                     zh:'以后也请多关照',                  en:'Please continue to support me' },
  { kr:'시간이 지나면 다 괜찮아질 거예요',             zh:'随着时间会好起来的',              en:'It will all be okay with time' },
  { kr:'지금 이 순간을 소중히 여기세요',               zh:'珍惜当下',                        en:'Cherish this moment' },
  { kr:'행복은 멀리 있는 게 아니에요',                 zh:'幸福并不遥远',                    en:'Happiness is not far away' },
  { kr:'매일 감사한 마음으로 살아요',                  zh:'每天怀着感恩的心生活',            en:'I live with gratitude every day' },
  { kr:'서로 이해하려고 노력하는 게 중요해요',         zh:'互相努力理解很重要',              en:'It is important to try to understand each other' },
  { kr:'문제를 해결하려면 대화가 필요해요',            zh:'要解决问题需要沟通',              en:'Dialogue is needed to solve problems' },
  { kr:'작은 실수로 큰 문제가 생길 수 있어요',         zh:'小错误可能导致大问题',            en:'Small mistakes can cause big problems' },
  { kr:'이 경험을 통해 많이 배웠어요',                 zh:'通过这次经历学到了很多',          en:'I learned a lot from this experience' },
  { kr:'앞으로의 계획을 세워 봤어요',                  zh:'制定了未来的计划',                en:'I made plans for the future' },
  { kr:'목표를 향해 한 걸음씩 나아가고 있어요',       zh:'一步步朝着目标前进',              en:'I am moving toward my goal step by step' },
  { kr:'포기하지 마세요, 반드시 이룰 수 있어요',       zh:'别放弃，一定能实现',              en:'Do not give up, you can achieve it' },
  { kr:'오늘도 좋은 하루 되세요',                      zh:'祝你今天也是美好的一天',          en:'Have a great day today' },
  { kr:'만나서 정말 반가웠어요',                       zh:'见到你真的很高兴',                en:'It was really nice meeting you' },
  { kr:'다음에 기회가 되면 또 뵙겠습니다',             zh:'下次有机会再见',                  en:'Hope to see you again when there is a chance' },
  { kr:'항상 응원하고 있어요',                         zh:'一直支持你',                      en:'Always cheering for you' },
  { kr:'함께여서 든든했어요',                          zh:'有你在很安心',                    en:'I felt reassured being with you' },
  { kr:'서로에게 좋은 친구가 되었으면 좋겠어요',       zh:'希望我们成为好朋友',              en:'Hope we become good friends' },
  { kr:'오늘 대화 정말 즐거웠어요',                    zh:'今天的对话真的很愉快',            en:'I really enjoyed our conversation today' },
  { kr:'다시 만날 날을 기대할게요',                    zh:'期待再次相见',                    en:'Looking forward to seeing you again' }
];
...
const LEVEL_META = {
  1: {
    icon:'🔤',
    zh:{ name:'初学者', desc:'学习韩文字母：辅音、元音、基础音节组合。建立发音基础。' },
    en:{ name:'Beginner', desc:'Learn Hangul: consonants, vowels, and basic syllable combinations.' }
  },
  2: {
    icon:'📚',
  ...
  3: {
    icon:'💬',
    zh:{ name:'日常短句', desc:'问候、感谢、道歉、请求等日常交流短语，可以简短回复。' },
    en:{ name:'Daily Phrases', desc:'Greetings, thanks, apologies, requests — enough to reply briefly.' }
  },
  4: {
    icon:'🗣️',
    zh:{ name:'简短对话', desc:'点餐、购物、问路、时间等场景的实用对话表达。' },
    en:{ name:'Short Dialogue', desc:'Ordering, shopping, directions, time — practical conversation.' }
  },
  5: {
    icon:'🎯',
    zh:{ name:'高级会话', desc:'表达意见、约定、情绪、建议。可以进行较好的日常交流。' },
  ...
  6: {
    ...
    en:{ name:'Fluent Communication', desc:'Complex expression, abstract discussion, life philosophy. Fluent level.' }
  }
};
...
const state = {
  level: 1,
  mode: 'image',        /* 'image' | 'text' | 'listen' | 'mix' */
  questions: [],
  current: 0,
  score: 0,
  correct: 0,
...
function getPool(level){
  if(level === 1){
    /* Level 1 uses syllables + letters */
    return [
      ...L1_CONSONANTS.map(x => ({ kr:x.kr, sub:x.roman, hint:x.name, group:'consonant' })),
      ...L1_VOWELS.map(x => ({ kr:x.kr, sub:x.roman, hint:x.name, group:'vowel' })),
      ...L1_SYLLABLES.map(x => ({ kr:x.kr, sub:x.roman, hint:x.roman, group:'syllable' }))
    ];
  }
...
function pickN(arr, n, exclude){
  const pool = arr.filter(x => !exclude || x !== exclude);
...
function buildQuestions(level, mode, count){
  ...
  const questions = [];
  const modes = mode === 'mix' ? ['image', 'text', 'listen'] : [mode];

  /* Ensure all questions are unique by KR token */
  ...
  while(questions.length < count && attempt < pool.length * 3){
    ...
    /* Options */
    const wrongCount = 3;
    const wrongs = pickN(pool, wrongCount, q);
    if(wrongs.length < wrongCount) continue;
...
window.startQuiz = function(mode){
  ...
  if(state.questions.length === 0){
    alert('Not enough content for this level.');
    return;
  }
...
function renderQuestion(){
  ...
  /* Render by mode */
  if(q.mode === 'image'){
    /* Show image, pick Korean */
    const hasImage = q.answer.img !== undefined;
    content.innerHTML = \`
      <div class="q-prompt">Look and choose the Korean</div>
      <div class="q-image">\${hasImage ? q.answer.img : '🔤'}</div>
      <div class="q-sub">\${hasImage ? '' : 'Which Korean matches?'}</div>
    \`;
    /* Answer options: Korean text */
    ...
    answers.className = 'answers cols-2';
    answers.innerHTML = q.options.map((opt, i) => {
      const meaning = opt.zh || opt.sub || opt.roman || '';
      return \`
        <button class="ans-btn" data-idx="\${i}">
          <span class="kr" style="color:inherit;font-size:15px">\${meaning}</span>
        </button>
      \`;
    }).join('');
  }
  else if(q.mode === 'listen'){
    /* Play audio, pick Korean */
    content.innerHTML = \`
    ...
    const sp = $('speakerBtn');
    if(sp){
      sp.addEventListener('click', () => speakKorean(q.answer.kr, false));
    }
    answers.className = 'answers cols-2';
    answers.innerHTML = q.options.map((opt, i) => \`
      <button class="ans-btn" data-idx="\${i}">
  ...
  /* Highlight level 1 hint for image mode */
  if(state.level === 1 && q.mode === 'image' && q.answer.sub){
    /* Show hint on the prompt */
    const prompt = content.querySelector('.q-prompt');
    if(prompt) prompt.textContent = 'Look and choose — ' + (q.answer.hint || q.answer.sub);
  }
}
...
function speakKorean(text, isAuto){
  ...
  try{
    ...
    /* Show/hide "no audio" hint if voice missing */
